...
void D_Capacitor::print(MyConsole* console) {
    ...
    valueStream << std::scientific << std::setprecision(3) << Value;
    std::string valueStr = valueStream.str();
    std::string temp = "*Capacitor(Name:" + Name + ", N+:" + N1 + ", N-:" + N2 + ", Value:" + valueStr + ")";
    console->log(temp);
}
...
std::vector<std::string>& D_Capacitor::getInfoList()
{
    ...
    std::ostringstream valueStream;
    valueStream << std::scientific << std::setprecision(3) << Value;
    std::string Value_str ="Value:"+valueStream.str();
