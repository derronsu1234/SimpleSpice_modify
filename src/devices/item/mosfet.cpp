...
void D_Mosfet::appendStampDC(AnalyzeContext* context) {
    ...
    {
        //console->log(fmt::format("For vgs {}, mos not open.", iterContext->vgs));
        return;
    }
...
void D_Mosfet::print(MyConsole* console) {
    //console->log(fmt::format("*Mosfet(Name:{}, Type:{}, Gate:{}, Base:{}, Drain:{}, Source:{})", Name, (int)Type, NG, NB, ND, NS));
}

std::vector<std::string>& D_Mosfet::getInfoList()
{
    std::string Type_str ="Type:"+Type;
    std::string NG_str ="Gate:"+NG;
    std::string NB_str ="Base:"+(NB);
    std::string ND_str ="Base:"+(ND);
    std::string NS_str ="Base:"+(NS);
    std::string Width_str ="Width:"+std::to_string(Width);
    std::string Length_str ="Length:"+std::to_string(Length);
    std::vector<std::string>* res = new std::vector<std::string>();
    res->push_back(Type_str);
    res->push_back(NG_str);
    res->push_back(NB_str);
    res->push_back(ND_str);
    res->push_back(NS_str);
    res->push_back(Width_str);
    res->push_back(Length_str);

    /*
    std::vector<std::string>* res = new std::vector<std::string>();
    res->push_back(fmt::format("Type:{}", (int)Type));
    res->push_back(fmt::format("Gate:{}", NG));
    res->push_back(fmt::format("Base:{}", NB));
    res->push_back(fmt::format("Drain:{}", ND));
    res->push_back(fmt::format("Source:{}", NS));
    res->push_back(fmt::format("Width:{}", Width));
    res->push_back(fmt::format("Length:{}", Length));*/
    return *res;
}
