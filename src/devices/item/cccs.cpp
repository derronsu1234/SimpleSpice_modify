...
D_CCCS::D_CCCS(std::string name, std::string n1, std::string n2, std::string nc1, std::string nc2, double value) 
    : BaseDevice(name), Value(value), N1(n1), N2(n2), NC1(nc1), NC2(nc2) {}

std::string D_CCCS::getDeviceType() {
    return "CCCS";
}

template<typename MatrixType, typename VectorType>
void D_CCCS::appendStamp(
    int &nodeCount, 
    MatrixType &mat, 
    VectorType &rhs,
...
void D_CCCS::print(MyConsole* console) {
    //std::string temp=fmt::format("*CCCS(Name:{}, N+:{}, N-:{}, NC+:{}, NC-:{}, Value:{:.3e})", Name, N1, N2, NC1, NC2, Value);
    //console->log(temp.c_str());
}

std::vector<std::string>& D_CCCS::getInfoList()
{
    std::string N1_str ="N+:"+N1;
    std::string N2_str ="N-:"+N2;
    std::string NC1_str ="NC+:"+NC1;
    std::string NC2_str ="NC-:"+NC2;
    std::string Value_str ="Value:"+std::to_string(Value);    
    std::vector<std::string>* res = new std::vector<std::string>();
    res->push_back(N1_str);
    res->push_back(N2_str);
    res->push_back(NC1_str);
    res->push_back(NC2_str);
    res->push_back(Value_str);
    //res->push_back(fmt::format("N+:{}", N1));
    //res->push_back(fmt::format("N-:{}", N2));
    //res->push_back(fmt::format("NC+:{}", NC1));
    //res->push_back(fmt::format("NC-:{}", NC2));
    //res->push_back(fmt::format("Value:{:.3e}", Value));
    return *res;
}
