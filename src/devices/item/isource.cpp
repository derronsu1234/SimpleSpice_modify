...
void D_ISource::appendStampAC(AnalyzeContext* context) {
    appendStamp(context->nodeCount, context->matc, context->rhsc, context->nodes, std::complex<double>(AC_Mag, (double)0));
}
...
void D_ISource::print(MyConsole* console) {
    //console->log(fmt::format("*Current Source(Name:{}, N+:{}, N-:{}, Value:{:.3e})", Name, N1, N2, DC_Value));
}
...
std::vector<std::string>& D_ISource::getInfoList()
{
    ...
    //res->push_back(fmt::format("Value:{:.3e}", DC_Value));

    std::string N1_str ="N+:"+N1;
    std::string N2_str ="N-:"+N2;
    std::string Value_str ="Value:"+std::to_string(DC_Value);
    std::vector<std::string>* res = new std::vector<std::string>();
