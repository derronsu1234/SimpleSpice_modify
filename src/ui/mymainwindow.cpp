...
void MyMainWindow::on_actionParse_triggered()
{
    ...
    if (!file) {
        QMessageBox::warning(this, tr("Error"), tr("Failed to open file!"),
                             QMessageBox::Ok);
        std::string temp="[SpParser] Can not open "+fileName.toStdString();
        console->log(temp);
        return;
    }
    ...
    yylineno = 1;
    std::string temp="[SpParser] Title: "+std::string(title);
    console->log(temp);
    circuit->title = title;
    ...
    if (ret == -255)
    {
        std::string temp1="[SpParser] node number: "+std::to_string(circuit->nodemap.size());
        console->log(temp1);
        std::string temp2="[SpParser] Parse success: "+std::to_string(circuit->devices.size());
        console->log(temp2);
    }
    else
    {
        ...
        console->log("[SpParser] Parse failed.");
        return;
    }

    ui.treeWidget->clear();
    for (auto& device : circuit->devices) {
        std::string title= device->Name + " [" + device->getDeviceType() + "]";
       // auto title = fmt::format("{} [{}]", device->Name, device->getDeviceType());
